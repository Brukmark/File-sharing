...
function UploadForm({ uploadBtnClick, progress }) {
  ...
  const onFileSelect = (file) => {
    ...
    if (file && file.size > 2000000) {
      ...
      setErrorMsg("maximum upload size is 2MB");
      return;
    }
    setErrorMsg(null);
    setFile(file);
  ...
  return (
    <div className="text-center">
      <div className="flex items-center justify-center w-full">
        <label
          for="dropzone-file"
          className="flex flex-col items-center justify-center w-full h-64 border-2 border-blue-300 border-dashed rounded-lg cursor-pointer bg-blue-50 dark:hover:bg-gray-600 dark:bg-gray-700 hover:bg-gray-100 dark:border-blue-600 dark:hover:border-gray-500 dark:hover:bg-gray-600"
        >
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            <svg
              className="w-12 h-12 mb dark:text-blue-500 text-blue-500"
              aria-hidden="true"
